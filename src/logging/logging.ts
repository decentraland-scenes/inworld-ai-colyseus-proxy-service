...
const DEFAULT_LOG_AS_JSON_SINGLE_LINE_ENABLED = true //process.env.LOGGING_AS_JSON_SINGLE_LINE_ENABLED !== undefined && process.env.LOGGING_AS_JSON_SINGLE_LINE_ENABLED === 'true'

export class LoggingConfiguration{
    static instance:LoggingConfiguration


    defaultLogAsJsonSingleLineEnabled:boolean = DEFAULT_LOG_AS_JSON_SINGLE_LINE_ENABLED
...
export class Logger {
    ...
    logAsJsonSingleLineEnabled:boolean = DEFAULT_LOG_AS_JSON_SINGLE_LINE_ENABLED
    ...
    logIt(level:LogLevel,method:string,msg:string, ...args:any[]){
        ...
        //TODO serlizes to single line
        if(this.logAsJsonSingleLineEnabled){
            //TODO consider this longer term https://github.com/well-known-components/logger/blob/main/src/console-printer.ts#L3-L22
            //include server time
            console.log( JSON.stringify(
                //TODO use object format like this https://www.baeldung.com/java-log-json-output
                [Date.now(),LogLevel[level] , contextStr , this.name , method ,":", msg , ...args]
                ))
        }else{
            console.log(LogLevel[level] , contextStr , this.name , method ,":", msg , ...args)
        }
        
    }
    /*getServerTimeISOFormatted(){
